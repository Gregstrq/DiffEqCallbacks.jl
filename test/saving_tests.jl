...
cb = SavingCallback((t,u,integrator)->u, saved_values, prob.u0)
...
cb = SavingCallback((t,u,integrator)->copy(u), saved_values, prob2D.u0)
...
cb = SavingCallback((t,u,integrator)->u[1], saved_values, prob2D.u0)
sol = solve(prob2D, Tsit5(), callback=cb)
@test all(idx -> sol.t[idx] == saved_values.t[idx], eachindex(saved_values.t))
...
cb = SavingCallback((t,u,integrator)->u, saved_values, prob.u0; saveat=saveat)
sol = solve(prob, Tsit5(), callback=cb)
...
cb = SavingCallback((t,u,integrator)->copy(u), saved_values, prob2D.u0; saveat=saveat)
sol = solve(prob2D, Tsit5(), callback=cb)
@test all(idx -> saveat[idx] == saved_values.t[idx], eachindex(saved_values.t))
...
saved_values = SavedValues(eltype(prob2D.tspan), eltype(prob2D.u0))
saveat = linspace(prob2D.tspan..., 10)
cb = SavingCallback((t,u,integrator)->u[1], saved_values, prob2d.u0; saveat=saveat)
sol = solve(prob2D, Tsit5(), callback=cb)
@test all(idx -> saveat[idx] == saved_values.t[idx], eachindex(saved_values.t))
...
cb = SavingCallback((t,u,integrator)->u, saved_values, prob.u0, saveat=saveat, tdir=-1)
...
cb = SavingCallback((t,u,integrator)->copy(u), saved_values, prob2D.u0; saveat=saveat, tdir=-1)
...
saveat = linspace(prob2D_inverse.tspan..., 10)
cb = SavingCallback((t,u,integrator)->u[1], saved_values, prob2D.u0; saveat=saveat, tdir=-1)
sol = solve(prob2D_inverse, Tsit5(), callback=cb)
@test all(idx -> saveat[idx] == saved_values.t[idx], eachindex(saved_values.t))
@test all(idx -> abs(sol(saveat[idx])[1] - saved_values.saveval[idx]) < 5.e-15, eachindex(saved_values.t))
